...
class FlappyBirdEnv(gym.Env):
    ...
    def __init__(self,is_demo=False):
        ...
        #self.observation_space = spaces.Box(low=0, high=255, shape=(self.screenheight, self.screenwidth), dtype=np.uint8)       
        ...
        #from moutain car
        #self.min_position = 0
        #self.low = np.array([self.min_position, -self.screenwidth])
        #self.high = np.array([self.screenheight, self.screenheight])
                
        #self.observation_space = spaces.Box(self.low, self.high, shape=(3,)) 
        ...
        #from simple flappy bird env - open ai gym
        ###
        # Observations - pipe height, bird velocity,
        #  distance to next pipe, height of next pipe,
        #  distance to second pipe, height of second pipe
        ###

        #self.observation_space = spaces.Box(low= np.array([0, -np.inf, -288, 0, -288, 0]),
         #                                   high = np.array([512, np.inf, 288, 512, 288, 0]))


    #return_type = 1 for tabular, 2 for simple nn, 3 for img data - deep learning    
    def step(self, input_actions, return_type=0):
        pygame.event.pump()
        pygame.event.set_grab(False)
        ...
        if self.lowerPipes[0]['x'] > -30:
        	myPipe = self.lowerPipes[0]
        else:
        	myPipe = self.lowerPipes[1]
			                
        image_data = pygame.surfarray.array3d(pygame.display.get_surface())
        return_data = [-self.playerx + myPipe['x'] ,-self.playery + myPipe['y'], self.playerVelY]
        #print self.upperPipes[0]['y'] + self.pipe_height - int(self.basey * 0.2)
        
        if return_type == 1: # tabular/grid world
            return - self.playerx + myPipe['x'], - self.playery + myPipe['y'], self.playerVelY, reward, terminal,{}
        
        if return_type == 2: #simple neural network
            return np.array(return_data), reward, terminal,{}
            
        if return_type == 3: #image data for deep neural network
